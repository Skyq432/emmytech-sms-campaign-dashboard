(async () => {
  ...
  function fallbackWhatsAppUrl() {
    const text = encodeURIComponent(config.whatsappClaimMessage || "Hello EmmyTech, I want to claim my 2 free spins.");
  ...
  function showManual(url, heading = "Continue to WhatsApp") {
    ...
    message.textContent = "Tap the button below to message EmmyTech and claim your 2 free spins.";
    manualLink.href = url;
  ...
  try {
    ...
    const whatsappText = encodeURIComponent(result?.whatsapp_message || config.whatsappClaimMessage || "Hello EmmyTech, I want to claim my 2 free spins.");
